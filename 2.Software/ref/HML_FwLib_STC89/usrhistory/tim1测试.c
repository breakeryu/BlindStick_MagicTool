...
void tim0_isr(void) __interrupt TF0_VECTOR
{
    static uint8_t i = 0;
    //重装载步骤是不能缺少的
    //TIM_setValue(PERIPH_TIM_1,TIM_calculateValue(5000, TIM_mode_1));
    TIM_setValue(PERIPH_TIM_0,64536);
    /* per 500ms */
    i++;
...
void tim1_isr(void) __interrupt TF1_VECTOR
{
    static uint8_t i = 0;
    //重装载步骤是不能缺少的
    //TIM_setValue(PERIPH_TIM_1,TIM_calculateValue(5000, TIM_mode_1));
    TIM_setValue(PERIPH_TIM_1,64536);
    /* per 500ms */
    i++;
    if (i == 12 && irq_signal == 1)
    {
        irq_signal = 0;
        i = 0;
    }
